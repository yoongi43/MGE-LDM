...
class MSELoss(LossModule):
    def __init__(self, key_a: str, key_b: str, weight: float = 1.0, mask_key: str = None, 
                 loss_mask_key: str = None,
                 name: str = 'mse_loss'):
        super().__init__(name=name, weight=weight)
    ...
    def forward(self, info):
        ...
        if self.loss_mask_key is not None and self.loss_mask_key in info:
            ## We don't calculate the loss for the masked out regions
            ## We remove the masked out regions when calculating the mean of the loss
            # mse_loss = mse_loss * info[self.mask_mult_key] # (B, 3C, T) * (B, 3C, 1)
            mask = info[self.loss_mask_key]
            mse_loss = mse_loss * mask

            T = mse_loss.shape[-1]
            mse_loss = mse_loss.sum() / (mask.sum()+1e-8) / T
